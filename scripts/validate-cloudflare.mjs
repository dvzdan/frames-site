...
if (build.includes("minute capstan after printing") || JSON.stringify(release).includes("minute capstan after printing")) errors.push("Obsolete minute-capstan cleanup note remains");
...
if (!assembly.includes("Already have your parts and images?") || !assembly.includes('href="#setup-inventory"') || !assembly.includes("Check parts &amp; start") || !assembly.includes('data-inventory-checkpoint') || !assembly.includes('href="/build/#plans"') || !assembly.includes("Print the parts") || !assembly.includes("Prepare the images")) errors.push("Assembly entry choices are incomplete");
if (!assembly.includes("If yours arrived prepared, no work is needed.") || !assembly.includes("single-hole punch ready for the Cover Image") || !assemblyScript.includes("bindAssemblyEntry") || !assemblyScript.includes('inventory.open = true')) errors.push("Assembly preflight checkpoint or compact image preparation is incomplete");
...
const requiredWeightAssemblySteps = [
  ...
  ["Install and Rotate the Latch", "05-secure-and-swing-latch.png"],
